...
def rollup_ro_status_from_items(order: RepairOrder) -> None:
    ...
    billed_out is never set here — only via bill_out_ro.
    """
    if (order.status or "").strip().lower() == "billed_out":
        return
...
def reopen_ro(order: RepairOrder, *, tech_id: str = "", tech_name: str = "") -> None:
    """
    Undo done (ready-to-bill) or mistaken billed-out — back on the shop floor.
    Puts the RO on the acting tech's queue when tech_id/name are given.
    """
    if order.status not in ("done", "billed_out"):
        raise ValueError("Only done or billed-out ROs can be reopened")
    order.done_at = ""
    order.billed_out_at = ""
    order.waiting_since = ""
    clear_current_task(order)
    ...
    for order in orders:
        ...
        for fi in d.get("found_issues") or []:
            if not isinstance(fi, dict):
                continue
            if str(fi.get("status") or "") == "pending":
        ...
        if cur_id or cur_name:
            ...
            job = (
                summarize_item_job(d, current_item)
                if current_item
                else {
                    "id": f"{d.get('id')}:",
                    "ro_id": d.get("id") or "",
