...
export function filterOthersEvents(
  ...
  return events.filter((e) => {
    // Desk-bound requests still ping this advisor PC even if dual-login / working
    // privilege filed them (Needs attention is the point of the notify).
    const deskRequest =
      e.type === "found_issue_created" || e.type === "next_day_requested";
    if (!deskRequest && isSelfEvent(e, self)) return false;
